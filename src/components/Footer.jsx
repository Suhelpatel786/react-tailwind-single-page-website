...
const Footer = () => (
  <section className={`${styles.flexCenter} ${styles.paddingY} flex-col`}>
    <div className={`${styles.flexStart} md:flex-row flex-col mb-8 w-full`}>
      ...
      <div className="flex-[1.5] w-full flex flex-row justify-between md:mt-0 mt-10 flex-wrap">
        {footerLinks.map((footerLink, index) => (
          <div
            key={footerLink.title}
            className="flex flex-col ss:my-0 my-4 m-w-[150px]"
          >
            <h4
              className={`font-poppins text-[18px] leading-[27px] text-white font-medium`}
            >
              {footerLink.title}
            </h4>
              ...
              {footerLink.links.map((link, i) => (
                <li
                  ...
                  className={`font-poppins font-normal ${
                    i === footerLink.links?.length - 1 ? "mb-0" : "mb-4"
